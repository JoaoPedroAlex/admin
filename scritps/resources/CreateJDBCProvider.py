# Authors: Sergio Stinchi, Andrea Minuto, Joao Pedro Alexandre (V1.0.7)

# Version        Description
# 1.0.7			 In the original script the command AdminTask.createJDBCProvider was wrong because the parameter 
#                -databaseType %s didn't have the delimeter '
#					Corrected by Joao Pedro ALexandre
...
try:
   ...
   #print "AdminTask.createJDBCProvider([-scope %s -providerType '%s' -databaseType '%s' -implementationType '%s' -name '%s' %s -classpath '%s' %s -isolated %s %s"  %(scopeStr,providerType,databaseType,implementationType,name,description,classpath,nativePath,isolated,implementationClassName) + "])"
   #in the original script the following line was wron because the parameter -databaseType %s didn't have the delimeter '
   AdminTask.createJDBCProvider(["-scope %s -providerType '%s' -databaseType '%s' -implementationType '%s' -name '%s' %s -classpath '%s' %s -isolated %s %s"  %(scopeStr,providerType,databaseType,implementationType,name,description,classpath,nativePath,isolated,implementationClassName)])
   print "OK"
except:
   print "KO"
   type, value, traceback = sys.exc_info()
   print "ERROR: %s (%s)" % (str(traceback), type)
