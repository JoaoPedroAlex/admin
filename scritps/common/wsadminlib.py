...
def parseargs():
#not valid in the current version of jython. Use getopt instead
#    from optparse import OptionParser
#
#   parser = OptionParser()
#   parser.add_option("-nodename", action="store", type="string", dest="nodename", help="name of the node")
#   parser.add_option("-servername", action="store", dest="servername", help="name of the server")
#   parser.add_option("-jndiejb", action="store", dest="jndiejb", help="jndi name for EJB")
#   parser.add_option("-contextroot", action="store", dest="contextroot", help="contextroot for application")
...
#  
    ...
    servers=[]
    try:
       options, args = getopt.getopt(sys.argv[1:], 'a:c:j:n:s:m:x:h', ["appname=", "contextroot=", "jndiejb=", "MapWebModToVH=", "nodename=", "servername=", "servers=", "clusters=", "help"])
    ...
    for o, a in options:
       if o in ("-h", "--help"):
          usage()
          sys.exit()
       elif o in ("-x", "--contextroot"):
          contextroot = a
	  arglist.extend( ['-contextroot',contextroot] )
       elif o in ("-a", "--appname"):
	  appname = a	
	  arglist.extend( ['-appname',appname] )
       elif o in ("-j", "--jndiejb"):
	  arglist.extend( ['-BindJndiForEJBNonMessageBinding', [['.*', '.*', '.*', a, "", ""]]] )	
       elif o in ("-m", "--MapWebModToVH"):
          arglist.extend( ['-MapWebModToVH', [['.*', '.*', a]]] )
       elif o in ("-n", "--nodename"):
          nodename = a
       elif o in ("-s", "--servername"):
          servername = a
       elif o == "--servers":
          servers = createServersDict(a)
       elif o in ("-c", "--clusters"):
	  clusternames = createClustersDict(a) 
       else:  
	  assert False, "unhandled option"
    if (appname == None) and (contextroot <> None):
       print 'You must enter the application name using argument --appname or -a' 
       sys.exit()
    if (nodename <> None) and (servername <> None):
       servers = [{'nodename':nodename,'servername':servername}]
    return ( filename, appname, servers, clusternames, contextroot, arglist )
...
def restartCluster( clustername, maxwait ):
    """Start the named server cluster"""
    m = "startCluster:"
    sop(m,"Start cluster %s" % clustername)
    cellname = getCellName()    # e.g. 'poir1Cell01'
    cluster = AdminControl.completeObjectName( 'cell=%s,type=Cluster,name=%s,*' % ( cellname, clustername ) )
    clusterMembers = listServersInCluster(cluster)
	...
	nodeName = AdminConfig.showAttribute( clusterMember, "nodeName" )
        ...
        restartServer(nodeName, serverName, maxwait)

    state = AdminControl.getAttribute( cluster, 'state' )
    #    rippleStart did not start the servers when they take a lot of time to stop
...
def syncCluster(clusterName):
    """Sync all servers (members) that are in the specified cluster"""
    m = "syncAllServersInCluster:"
    sop(m,"clusterName = %s" % clusterName)    
    clusterMembers = listServersInCluster(clusterName)
    for clusterMember in clusterMembers:
	nodeName = AdminConfig.showAttribute( clusterMember, "nodeName" )
	sop(m, "Syncing Node %s" % (nodeName ))	
	sync(nodeName)
...
def reStartApplicationOnServer(appname,nodename,servername):
    """Start the named application on one servers"""
    m = "restartApplicationOnServer:"
    sop(m,"Entry. appname=%s nodename=%s servername=%s" % ( appname,nodename,servername ))
    cellname = getCellName()
    # Get the application manager MBean
    appManager = AdminControl.queryNames('cell=%s,node=%s,type=ApplicationManager,process=%s,*' %(cellname,nodename,servername))
    ...
    sop(m,"Exit. rc=%s" % ( repr(rc) ))
    while isApplicationRunning(appname):
	time.sleep( 30 )
    # start it
    rc = AdminControl.invoke(appManager, 'startApplication', appname)
    sop(m,"Exit. rc=%s" % ( repr(rc) ))
...
def installApplication( filename, servers, clusternames, options ):
	...
	ex: servers = [{'nodename':'TPNode01','servername':'tp_dev_01'}]
...
def updateApplication(filename, appname, servers, clusternames, options):
    ...
    if appname == None:
       ...
       try:
           ...
           print 'ERROR: Did not find application name in file %s' % filename
           print 'Application name must be given as argument'
       else:	
           # parse the xml file
           # (cheat - it's simple)
    ...
    arglist = ['-operation', 'update', '-update.ignore.old', '-contents', filename, '-target', target]
    ...
    sop(m,"Calling AdminApp.update of %s with arglist = %s" % ( filename, repr(arglist) ))
    AdminApp.update( appname, 'app', arglist )
...
def getNode(wasHome):
	configdir = os.path.join(wasHome, 'config/cells')
	filenames = os.listdir(configdir)
	file = filenames[0]
	f = os.path.join(configdir, file, '/nodes')
	filenames = os.listdir(f)
	nodename = filenames[0]
	return nodename 
...
def scheduleJob(jobType, targetNode, jobParams, activationTime):
    """Submits a job to the Job Manager.
    ...
    - String jobParams - parameters specific to the jobType
        For example, createProxyServer requires a string which looks like a list of lists: "[[serverName proxy1] [nodeName ding3Node]]"
    - String activationTime - parameter to schedule the job to a specific date and time
        For example, '2016-03-12T14:00:00'
    Results:
    - Returns a string jobToken, which is a big number.  eg, 119532775201403043
    - This number is useful to subsequently query status of the submitted job."""
    m = "submitJob:"
    sop(m,"Entry. jobType=%s targetNode=%s jobParams=%s" % (jobType, targetNode, jobParams))
    # wsadmin>jobToken = AdminTask.submitJob('[-jobType createProxyServer -targetList [ding3Node03] -jobParams "[serverName proxy01 nodeName ding3Node]"]')
    argString = '[-jobType %s -targetList [%s] -jobParams "%s" -activationDateTime %s]' % (jobType, targetNode, jobParams, activationTime)
    sop(m,"Calling AdminTask.submitJob with argString=%s" % ( argString ))
    jobToken = AdminTask.submitJob( argString )
    sop(m,"Exit. Returning jobToken=%s" % ( jobToken ))
    return jobToken
...
def waitForJobSuccess(jobToken,timeoutSecs):
    ...
    # the following code is to avoid more than 4 minutes wait
    '''
...
def sync(nodename):
   """SyncNode config to a node    - return 0 on success, non-zero on error"""
   m = "wsadminlib.syncNode"
   ...
   returncode = 0
       ...
       # Note: Is not the dmgr node - if it did, we'd
       # have to skip it
       # We do, however, have to skip unmanaged nodes.  These will show up
       # when there is a web server defined on a remote machine.
   if not nodeIsDmgr( nodename ) and not nodeIsUnmanaged( nodename ):
...
def createServersDict(propString):
    """Helper method converts a flat string which looks like a list of properties
    into a dictionary of keys and values.

    Expects input of the form   value0,value1,value2,value3,
    Returns output of the form  [{'nodename':'value0','servername':'value1'}{'nodename':'value2','servername':'value3'}]
    Raises an exception if the original string does not look like a list of properties."""
    m = "createServersDict: "
    # print "%s Entry. propString=%s" % ( m, propString, )
    # Convert the single long string to a list of strings.
   ...
   # print "%s Split string into list. stringList=%s" % ( m, stringList, )
    ...
    # Transfer each value string into a dictionary
    
    servers = []	
    i = 0
    while i < len(stringList):
        dict = {}
	dict['nodename'] = stringList[i]
	dict['servername'] = stringList[i+1]
	#print dict
	servers.append(dict)
	i = i+2
...
def createClustersDict(propString):
    """Helper method converts a flat string which looks like a list of properties
    into a dictionary of keys and values.
