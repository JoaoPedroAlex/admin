...
f = open(outputPath, "w")
washome = sys.argv[0]
isWindows = isWindows(washome)
commands = []
inputPath = '%s/JDBCProvider' % (inputPath)
for filename in os.listdir(inputPath):
   if isWindows: 
      commands.append("%%WAS_HOME%%\\bin\wsadmin.bat -lang jython -f %s\CreateJDBCProvider.py %s\%s -javaoption \"-Dwsadmin.script.libraries=%s\"" % (resourcesPath,inputPath,filename,infoPath)) 
